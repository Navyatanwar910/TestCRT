...
Verify Transfer of a participant beforing admitting to another program
    ...
    ClickText    New
    ClickText    Contact    partial_match=False
    TypeText    Search for a contact...    Aakash Arora
    ClickText                        Aakash Arora
    ClickText    Stage
    ClickText    Select an Option
    ClickText    Pending                   recognition_mode=vision
    ClickText    Save
    ClickText    Enrollment                anchor=Overview
    ClickElement       xpath=//tr[.//a[contains(text(),'Aakash Arora')]]//button[contains(@class,'slds-button') or contains(@title,'Actions')]
